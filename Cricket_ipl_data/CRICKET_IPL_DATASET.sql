...
SELECT PLAYER_NAME,TEAM,ROLE, SALARY 
FROM IPL_PLAYERS
...
SELECT AVG(RUNS) FROM IPL_PLAYERS;

SELECT * FROM IPL_PLAYERS
WHERE RUNS > (SELECT AVG(RUNS) FROM IPL_PLAYERS);


-- 4. Players in Same Team as Virat Kohli

SELECT * FROM IPL_PLAYERS 
WHERE TEAM IN (SELECT TEAM FROM IPL_PLAYERS 
...
SELECT *
FROM IPL_PLAYERS 
WHERE  WICKETS = ( SELECT  MAX(WICKETS) FROM IPL_PLAYERS);
...
SELECT * FROM IPL_PLAYERS
WHERE SALARY > (SELECT AVG(SALARY) FROM IPL_PLAYERS)
...
FROM IPL_PLAYERS 
WHERE ROLE IN (SELECT ROLE FROM IPL_PLAYERS 
...
SELECT MAX(SALARY) FROM ipl_players;
...
SELECT AVG(WICKETS) FROM IPL_PLAYERS;
...
SELECT TEAM , COUNT(PLAYER_NAME) AS 'TOT_PLAYERS'
FROM IPL_PLAYERS 
GROUP BY TEAM
HAVING TOT_PLAYERS > 3
...
SELECT PLAYER_NAME, SALARY, ROLE
FROM IPL_PLAYERS
...
SELECT * FROM IPL_PLAYERS
WHERE ROLE NOT IN ('BOWLER');

SELECT * FROM IPL_PLAYERS
WHERE ROLE NOT IN (SELECT ROLE FROM IPL_PLAYERS WHERE ROLE = 'BOWLER');
...
-- 21. FIND TOTAL RUNS WITHOUT GROUP BY

SELECT PLAYER_NAME , TEAM , RUNS ,
	SUM(RUNS) OVER (ORDER BY RUNS) AS TOT_RUNS
	FROM IPL_PLAYERS;
 
 -- PARTITION BY TEAMS
 SELECT PLAYER_NAME , TEAM , RUNS ,
	SUM(RUNS) OVER (PARTITION BY TEAM  ORDER BY RUNS) AS TOT_RUNS
	FROM IPL_PLAYERS;
 

-- RUNS WISE RANK
SELECT player_NAME, runs,
RANK() OVER (ORDER BY runs DESC) AS RANKK
FROM ipl_players;

-- BY DENSE RNAK
SELECT PLAYER_NAME, runs,
DENSE_RANK() OVER (ORDER BY runs DESC) AS RANKK
FROM ipl_players;
 
 -- DESC RUNWISE ROW NO
SELECT PAYER_NAME, RUNS,
